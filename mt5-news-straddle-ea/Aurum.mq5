...
//|      alla distanza calcolata come sopra, e continua a SPOSTARLI    |
//|      (non li ricrea, li modifica) seguendo il range che si        |
//|      allarga, finche' non arriva a 3 secondi dall'orario: da li'   |
//|      in poi il range e i livelli restano fermi (congelati) fino    |
//|      all'esecuzione o alla scadenza;                               |
...
//+------------------------------------------------------------------+
//| Calcola i livelli di entrata/SL/TP dal range attuale (distanza in |
//| DOLLARI, ricalcolata live dal cambio EURUSD/oro) rispettando la   |
//| distanza minima del broker (STOPS_LEVEL). Usata sia per piazzare  |
//| gli ordini la prima volta sia per ri-prezzarli mentre li si       |
//| insegue.                                                          |
//+------------------------------------------------------------------+
void ComputeOrderLevels(double &buyPrice, double &sellPrice,
                         ...
                         double &dist)
  {
   ...
   dist = GetEquivalentGoldDistance();
...
//| a SECONDS_ARM_EARLY dalla notizia - distanze in DOLLARI diretti,  |
//| niente calcolo di "pip"                                           |
//+------------------------------------------------------------------+
void PlaceOrders()
  {
   ...
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   double buyPrice, sellPrice, sl_buy, tp_buy, sl_sell, tp_sell, dist;
   ComputeOrderLevels(buyPrice, sellPrice, sl_buy, tp_buy, sl_sell, tp_sell, dist);

   double volume = NormalizeVolume(InpLotSize);
   datetime expiration = g_event.time + EXPIRATION_MINUTES * 60;

   PrintFormat("Aurum: distanza equivalente a %.1f pip EURUSD -> %.2f$ sull'oro. Range [%s , %s], BuyStop=%s SellStop=%s (inseguimento attivo fino a %ds dalla notizia)",
   ...
   if(g_event.buyTicket != 0 && OrderStillPending(g_event.buyTicket) &&
      MathAbs(buyPrice - g_event.buyPricePlaced) >= point)
     {
      if(trade.OrderModify(g_event.buyTicket, buyPrice, sl_buy, tp_buy, ORDER_TIME_SPECIFIED, expiration))
         ...
         PrintFormat("Aurum: errore spostamento BuyStop: %d %s", trade.ResultRetcode(), trade.ResultRetcodeDescription());
     }
   if(g_event.sellTicket != 0 && OrderStillPending(g_event.sellTicket) &&
      MathAbs(sellPrice - g_event.sellPricePlaced) >= point)
     {
      if(trade.OrderModify(g_event.sellTicket, sellPrice, sl_sell, tp_sell, ORDER_TIME_SPECIFIED, expiration))
         ...
         PrintFormat("Aurum: errore spostamento SellStop: %d %s", trade.ResultRetcode(), trade.ResultRetcodeDescription());
     }
  }
