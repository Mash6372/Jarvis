...
router.get('/', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM ecostazioni ORDER BY comune, nome').all();
  res.json(rows);
});

router.get('/:id', async (req, res) => {
  const row = await db.prepare('SELECT * FROM ecostazioni WHERE id = ?').get(req.params.id);
...
router.post('/', async (req, res) => {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM ecostazioni WHERE id = ?').get(info.lastInsertRowid));
});

router.put('/:id', async (req, res) => {
  const existing = await db.prepare('SELECT * FROM ecostazioni WHERE id = ?').get(req.params.id);
  ...
  res.json(await db.prepare('SELECT * FROM ecostazioni WHERE id = ?').get(req.params.id));
});

router.delete('/:id', async (req, res) => {
  await db.prepare('DELETE FROM ecostazioni WHERE id = ?').run(req.params.id);
  res.status(204).end();
});
