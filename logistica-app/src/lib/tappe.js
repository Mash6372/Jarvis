...
function posizioneTappa(tappa) {
  if (tappa.tipo === 'ritiro' && tappa.ecostazione_id) {
    const e = db.prepare('SELECT * FROM ecostazioni WHERE id = ?').get(tappa.ecostazione_id);
    if (!e) return null;
  ...
  if (tappa.tipo === 'scarico' && tappa.impianto_id) {
    const im = db.prepare('SELECT * FROM impianti WHERE id = ?').get(tappa.impianto_id);
    if (!im) return null;
